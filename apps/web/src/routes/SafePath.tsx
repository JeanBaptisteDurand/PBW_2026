...
export default function SafePath() {
  ...
  useEffect(() => {
    ...
    api.getSafePathRun(runId).then((saved) => {
      ...
      if (saved.resultJson?.corridor) {
        setLiveCorridor(saved.resultJson.corridor);
        addCorridorSkeleton(saved.resultJson.corridor);
      }
      if (saved.resultJson?.corridorAnalysis) {
        addCorridorPaths(saved.resultJson.corridorAnalysis);
      }
    }).catch(() => {
  ...
  const downloadReport = useCallback(() => {
    ...
    a.href = url;
    a.download = `xrplens-safepath-${srcCcy}-${dstCcy}-${Date.now()}.md`;
    a.click();
              ...
              <div className="flex justify-between items-start flex-wrap gap-3">
                <div>
                  <div className="text-[9px] font-bold tracking-[2px] uppercase text-xrp-500 mb-1">
                    XRPLens Safe Path Compliance Report
                  </div>
