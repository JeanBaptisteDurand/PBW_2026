// xrplens/apps/web/src/lib/historyGraphAdapter.tsx
//
...
// XRPLens analyze graph, plus a concentric-ring radial layout so the graph
...
//
// Colours mirror `NODE_COLORS` / `EDGE_COLORS` from @xrplens/core so the
// history page "feels" like the analyze page.

import React from "react";
import type { Node, Edge, ReactFlowState } from "reactflow";
import { Position, MarkerType, useStore } from "reactflow";
import { NODE_COLORS as CORE_NODE_COLORS, EDGE_COLORS as CORE_EDGE_COLORS } from "@xrplens/core";
...
// ─── Color palette ────────────────────────────────────────────────────────
//
// Borders are sourced from @xrplens/core NODE_COLORS / EDGE_COLORS so the
...
// Each branch maps back to a @xrplens/core EDGE_COLORS swatch so trustlines,
...
function edgeColor(txType: string): string {
  ...
  if (t.startsWith("ammdeposit") || t.startsWith("ammwithdraw") || t.startsWith("ammvote") || t.startsWith("ammbid") || t === "lp holder")
    return CORE_EDGE_COLORS.PROVIDES_LIQUIDITY;
  if (t.startsWith("offer") || t.startsWith("offer (ask)") || t.startsWith("offer (bid)")) return CORE_EDGE_COLORS.TRADES_ON;
...
export function adaptHistoryNodes(
  nodes: HistoryNode[],
  selectedId?: string,
): Node[] {
          ...
          <div
            style={{
              ...
              border: `${isSelected ? 2.5 : 1.5}px solid ${
                isSelected ? "#f8fafc" : isError ? "#ef4444" : colors.border + (isLight ? "80" : "cc")
              }`,
        ...
        {depthConfigs.map((cfg) => {
          ...
          return (
            <g key={cfg.depth}>
              ...
              <g
                transform={`translate(${cx}, ${cy - cfg.outerRadius - 20})`}
              >
                <rect
                  x={-180}
                  y={-28}
