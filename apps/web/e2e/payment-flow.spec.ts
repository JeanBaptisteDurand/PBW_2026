...
test.describe("Payment Gate Flow", () => {
  test.beforeEach(async () => {
    ...
    execSync(
      `docker exec xrplens-postgres-1 psql -U xrplens -d xrplens -c "DELETE FROM \\"PremiumSubscription\\"; DELETE FROM \\"PaymentRequest\\"; DELETE FROM \\"User\\";"`,
    );
  });
  ...
  test("demo wallet payment unlocks premium features", async ({ page }) => {
    ...
    await page.evaluate(() => localStorage.removeItem("xrplens_auth"));
    await page.reload();
