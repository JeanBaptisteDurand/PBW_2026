...
export function generateCorridorAiNoteLocal(
  ...
  if (entry.category === "off-chain-bridge") {
    ...
    return (
      ...
      `XRPL hop: ${entry.bridgeAsset ?? "RLUSD"}. XRPLens does not path_find this lane — there is no on-chain IOU trust line — so the verdict below is derived from real-world partner quality, not on-ledger depth. ` +
      verdict
    );
  }
