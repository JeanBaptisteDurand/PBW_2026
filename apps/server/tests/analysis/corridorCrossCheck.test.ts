...
import type { CorridorRequest } from "@xrplens/core";

// ─── Cross-check: XRPLens vs raw XRPL rpc ────────────────────────────────
//
// Purpose: prove that the CorridorAnalysis XRPLens returns matches what a
...
//
// If (3) diverges, XRPLens is lying about the risk profile — this test
...
describe("Corridor cross-check: XRPLens matches raw XRPL rpc", () => {
  ...
  it(
    "USD.Bitstamp → RLUSD: XRPLens risk flags match raw account_info flags",
    async () => {
      ...
      // 2. Run XRPLens corridor analysis end-to-end.
      ...
      expect(
        clawbackFlag,
        "XRPLens must flag the RLUSD hop as CLAWBACK_ENABLED to match the raw ledger",
      ).toBeDefined();
