...
The MCP server lets Claude (Desktop or Code) talk to XRPLens directly. Instead of copy-pasting data into chat, the user asks Claude a question and Claude calls the XRPLens API behind the scenes.
...
MCP server receives the tool call via stdio
        |
        v
MCP server calls the XRPLens REST API (same API the web app uses)
        |
        v
...
Users download a zip from the docs page (`/xrplens-mcp.zip`). The zip contains:
...
```bash
cd xrplens/apps/mcp-server
npm run build:zip
cp xrplens-mcp.zip ../web/public/xrplens-mcp.zip
```
...
3. Zips everything into `xrplens-mcp.zip`
...
2. Downloads `xrplens-mcp.zip` (138KB)
3. Unzips anywhere on their computer
4. Opens `.env`, pastes their API key (`xlens_...`)
5. Adds to Claude config:
   - **Claude Code**: `claude mcp add xrplens -- node /path/to/xrplens-mcp/server.js`
...
XRPLENS_API_URL=http://localhost:3001/api npm run dev
