...
Connect Claude to XRPLens — browse corridors, run entity audits, query RAG, and check live DEX depth through natural conversation.

## Setup (3 steps)

### 1. Add your API key

Open the `.env` file in this folder and replace `xxxxxx` with your XRPLens API key:

```
XRPLENS_API_KEY=xlens_paste_your_key_here
```

You can get an API key from your XRPLens account page: https://xrplens.dev/account
...
```bash
claude mcp add xrplens -- node /full/path/to/xrplens-mcp/server.js
```
...
```json
{
  "mcpServers": {
    "xrplens": {
      "command": "node",
      "args": ["/full/path/to/xrplens-mcp/server.js"],
      "env": {
        "XRPLENS_API_KEY": "xlens_paste_your_key_here"
      }
    }
...
Replace `/full/path/to/xrplens-mcp/server.js` with the actual path where you unzipped this folder.
