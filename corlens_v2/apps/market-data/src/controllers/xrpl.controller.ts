...
export async function registerXrplRoutes(app: FastifyInstance, svc: XrplService): Promise<void> {
  ...
  }, async (req) => svc.accountTx(req.params.address, req.query.limit, req.query.sinceUnixTime) as Promise<unknown[]>);
  ...
  }, async (req) => svc.bookOffers(req.query.takerGetsCurrency, req.query.takerGetsIssuer, req.query.takerPaysCurrency, req.query.takerPaysIssuer, req.query.limit) as Promise<Raw>);

  typed.get("/xrpl/amm/by-pair", {
    schema: { querystring: md.AmmByPairQuery, response: { 200: RawResponse }, tags: ["xrpl"] },
  }, async (req) => svc.ammByPair(req.query.asset1Currency, req.query.asset1Issuer, req.query.asset2Currency, req.query.asset2Issuer) as Promise<Raw>);
  ...
  }, async (req) => svc.pathFind(req.body.sourceAccount, req.body.destinationAccount, req.body.destinationAmount) as Promise<Raw>);
}
