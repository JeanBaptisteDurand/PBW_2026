...
import type { ChatMessage } from "../../lib/core-types.js";
...
export function ChatInterface({ analysisId }: ChatInterfaceProps) {
  ...
  // chatId tracking removed in v2 — the path service auto-creates/reuses
  // the latest chat per analysisId on every POST /api/analysis/:id/chat.
  const [input, setInput] = useState("");
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const bottomRef = useRef<HTMLDivElement>(null);

  // Auto-scroll to bottom — depending on messages.length + isLoading is
  // intentional (the ref only updates after layout, so we re-trigger when
  // either changes).
  // biome-ignore lint/correctness/useExhaustiveDependencies: ref-driven side-effect, deps are by-design.
  useEffect(() => {
  ...
  const sendMessage = async (text: string) => {
    ...
    try {
      const res = await api.path.chat(analysisId, text.trim());
  ...
  return (
    ...
    >
      ...
      >
        ...
        {messages.map((msg, i) => (
          <div
            // biome-ignore lint/suspicious/noArrayIndexKey: append-only message log, index is stable.
            key={i}
            style={{
              display: "flex",
      ...
      >
        <form onSubmit={handleSubmit} style={{ display: "flex", gap: 8 }}>
          <Input
            ...
            className="flex-1"
          />
          <Button type="submit" disabled={isLoading || !input.trim()} style={{ flexShrink: 0 }}>
            Send
          </Button>
        </form>
      </div>
