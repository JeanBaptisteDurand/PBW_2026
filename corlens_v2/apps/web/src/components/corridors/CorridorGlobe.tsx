...
// cobe 0.6.4 — installed version — does not export an `Arc` type and the
// COBEOptions surface does not include `arcs`. v1 SPA depended on an older
// fork that exposed both. To keep the port mechanical, we declare a minimal
// local Arc shape and cast options through to the createGlobe call. Arcs
// don't render in v0.6.4 (the data is harmless), so the globe shows as the
// dotted earth + markers only — full v1 visual fidelity will return once
// cobe's arcs API is restored or we swap to a fork.
type Arc = {
  from?: number[];
  to?: number[];
...
export function CorridorGlobe({
  ...
  useEffect(() => {
    ...
    // cobe 0.6.4 typed COBEOptions doesn't include arcs / arcColor / etc.;
    // pass through with a cast (the v2 build doesn't render them anyway —
    // see the local Arc declaration above).
    ...
    const tick = (now: number) => {
      ...
      // cobe 0.6.4 Renderer's `.update` exists at runtime but isn't on the
      // type. Cast through to keep the v1 animation loop intact.
      (globe as unknown as { update: (opts: Record<string, number>) => void }).update({
        phi: phiRef.current,
        theta: thetaRef.current,
        width: width * dpr,
        height: height * dpr,
