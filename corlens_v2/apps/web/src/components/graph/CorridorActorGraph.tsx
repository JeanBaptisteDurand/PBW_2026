...
import type { CorridorActor, CorridorPairDef } from "../../lib/core-types.js";
        ...
        label: (
          <div className="flex flex-col px-2 py-1 text-left">
            <div className="flex items-center gap-1 truncate">
              <span className="font-semibold text-white text-[11px] truncate">{actor.name}</span>
              {actor.country && (
                <span className="ml-auto font-mono text-[9px] text-slate-500">{actor.country}</span>
              )}
            </div>
    ...
    data: {
      label: (
        <div className="flex flex-col items-center justify-center px-3 py-2">
          <div className="text-[9px] uppercase tracking-widest text-emerald-400/80">XRPL hop</div>
          <div className="mt-1 font-mono text-base font-bold text-emerald-300">{bridge}</div>
...
const NODE_LEGEND: NodeLegendEntry[] = [
  { label: "source fiat", fill: "#0f172a", border: "#0ea5e9", title: "Source currency (sky ring)" },
  {
    ...
    fill: "#042f2e",
    border: "#10b981",
    title: "XRPL bridge asset — RLUSD / USDC / XRP (emerald ring)",
  },
  {
    label: "dest fiat",
    fill: "#0f172a",
  ...
  },
  {
    label: "CEX",
    fill: ACTOR_FILL.cex,
    border: ACTOR_BORDER.cex,
    title: "Licensed centralised exchange",
  },
  {
    label: "ODL",
    fill: ACTOR_FILL.odl,
    border: ACTOR_BORDER.odl,
    title: "Ripple ODL / Ripple Payments partner",
  },
  {
    label: "bank",
    fill: ACTOR_FILL.bank,
    border: ACTOR_BORDER.bank,
    title: "Bank or qualified custodian",
  },
  {
    label: "hub",
    fill: ACTOR_FILL.hub,
    border: ACTOR_BORDER.hub,
    title: "Cross-country ODL super-hub (Tranglo, Onafriq, …)",
  },
  {
    label: "remittance",
    fill: ACTOR_FILL.remittance,
    border: ACTOR_BORDER.remittance,
    title: "Remittance operator",
  },
  {
    label: "fintech",
    fill: ACTOR_FILL.fintech,
    border: ACTOR_BORDER.fintech,
    title: "E-money / BaaS / card fintech",
  },
  {
    label: "mobile-money",
    fill: ACTOR_FILL["mobile-money"],
    border: ACTOR_BORDER["mobile-money"],
    title: "Mobile-money bridge (M-Pesa / MTN / Orange)",
  },
  {
    label: "OTC",
    fill: ACTOR_FILL.otc,
    border: ACTOR_BORDER.otc,
    title: "OTC desk / institutional market-maker",
  },
...
const EDGE_LEGEND: EdgeLegendEntry[] = [
  {
    label: "RLUSD leg",
    color: COLORS.edgeRlusd,
    title: "Actor holds RLUSD on XRPL — the preferred bridge leg",
  },
  {
    label: "ODL leg",
    color: COLORS.edgeStrong,
    title: "Ripple ODL partner — bridges via XRP if no RLUSD",
  },
...
function EdgeDot({ entry }: { entry: EdgeLegendEntry }) {
  return (
    <span className="inline-block h-[2px] w-4 flex-shrink-0" style={{ background: entry.color }} />
  );
}

export function CorridorActorGraph({ corridor, height = 480 }: CorridorActorGraphProps) {
