...
import type { CorridorDetailResponse, CorridorPathHop } from "../../lib/core-types.js";
...
export function CorridorRoutesGraph({
  ...
  const { rfNodes, rfEdges, counts } = useMemo(() => {
    ...
    for (const col of byColumn.values()) {
      col.sort(
        (a, b) =>
          (a.issuerName ?? "").localeCompare(b.issuerName ?? "") || a.key.localeCompare(b.key),
      );
    }
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div>
          {counts.sources} source{counts.sources !== 1 ? "s" : ""} × {counts.dests} dest
          {counts.dests !== 1 ? "s" : ""} · {counts.hops} intermediate · {counts.edges} edges
        </div>
