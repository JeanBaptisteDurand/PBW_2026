// ─── v1 @corlens/core port (verbatim) ──────────────────────────────────
//
// This file is the v1 `packages/core/src/index.ts` copied across so the
// ported v1 React components (~14 kLOC) keep their imports unchanged. The
// v2 Zod-backed source of truth is `@corlens/contracts` — when a runtime
// value crosses the network boundary it is validated there; the types
// here only shape what the SPA's React components manipulate after the
// API client has handed them a parsed payload.
//
// Two adapters in `apps/web/src/api/index.ts` and
// `apps/web/src/hooks/useGraph.ts` bridge the v2 wire shapes (`nodeId`,
// `nodeCount`, list responses without wrappers, etc.) to the v1 shapes
// declared below. Search for `enrichListItem`, `enrichDetail`, and
// `adaptGraph` to see the bridge points.
//
// Do not edit this file without also auditing the bridge. If you find
// yourself adding fields here to please the component code, prefer
// instead either (a) updating the bridge to default-fill them, or
// (b) updating the v1 component to consume `@corlens/contracts` directly.
