...
import type { LabelContent, LabelPlacement, LabelStyle } from "../scene/sceneTypes";
...
export function ContentOverlay({ item, viewIndex, isExiting = false }: ContentOverlayProps) {
  ...
  const cardStyle = {
    ...
    "--landing-label-title-fill": style.titleGradient ? "transparent" : "currentColor",
    ...
    <>
      {showTitle ? (
        useGradientStrokeTitle ? (
          <h1
            className="landing-label-title landing-label-title--stroke-gradient"
      ...
      {showBody ? (
        item.bodyHtml ? (
          // biome-ignore lint/security/noDangerouslySetInnerHtml: bodyHtml is sourced from our own static content files (Landing/content/*.ts), no user input.
          <p className="landing-label-hint" dangerouslySetInnerHTML={{ __html: item.bodyHtml }} />
        ) : (
