...
export function createScene(canvas: HTMLCanvasElement, config: SceneConfig) {
  ...
  const keyLight = new DirectionalLight(config.lighting.key.color, config.lighting.key.intensity);
...
function updateStarfield(
  ...
  for (let i = 0; i < phases.length; i += 1) {
    ...
    const offsetY = Math.cos(time * speed + phase) * amplitude;

    positions[baseIndex] = (base[baseIndex] ?? 0) + offset;
    positions[baseIndex + 1] = (base[baseIndex + 1] ?? 0) + offsetY;
    positions[baseIndex + 2] = base[baseIndex + 2] ?? 0;
  }
...
function getCoveredViewport(containerWidth: number, containerHeight: number, targetAspect: number) {
