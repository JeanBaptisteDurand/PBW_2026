...
import { AnalysisStatusCard } from "../fragments/Analyze/AnalysisStatusCard.js";
import { AnalyzeIntro } from "../fragments/Analyze/AnalyzeIntro.js";
import { QuickStartPresets } from "../fragments/Analyze/QuickStartPresets.js";
import { useAnalysisHistory, useAnalysisStatus, useStartAnalysis } from "../hooks/useAnalysis.js";
...
function RecentAudits() {
  const { data } = useAnalysisHistory();
  const done = data?.analyses.filter((a) => a.status === "done") ?? [];
          ...
          {done.slice(0, 10).map((a) => (
            ...
            >
              <div className="min-w-0">
                ...
                <div className="text-[11px] text-slate-500 font-mono truncate">{a.seedAddress}</div>
              </div>
      ...
      <Card>
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            {/* Depth selector */}
            <div>
              <span className="block text-xs font-medium text-slate-400 mb-1.5">Crawl Depth</span>
              ...
              {depth >= 2 && (
                <p className="text-[11px] text-amber-400/80 mt-2">
                  Deep mode runs multiple XRPL crawls concurrently. Safe-capped at 60 crawls / 800
                  nodes. Chat and Compliance will see every crawled hub as full context.
                </p>
              )}
            </div>
