// corlens/apps/web/src/routes/History.tsx
...
function HistoryInner({ embedded }: { embedded?: boolean }) {
  ...
  const rfEdges = useMemo(() => adaptHistoryEdges(Array.from(state.edges.values())), [state.edges]);
  ...
  const selectedNode = state.selectedNodeId ? state.nodes.get(state.selectedNodeId) : undefined;
      ...
      <div className="max-w-[1400px] mx-auto px-6 py-10">
        ...
        <div className="mb-6">
          <div className="text-xs font-bold uppercase tracking-widest text-xrp-400 mb-2">
            CorLens · Account History
          </div>
          <h1 className="text-3xl font-bold text-white mb-2">History Graph</h1>
          <p className="text-slate-400 text-sm max-w-3xl">
            Crawl an XRPL account's transaction history and visualise the account graph. Depth 1
            shows direct counterparties; depth 2–3 expands into their networks.
          </p>
        </div>
                  ...
                  {state.stats && (
                    <div className="mt-3 p-2 bg-emerald-500/10 border border-emerald-500/30 rounded text-xs space-y-1">
                      ...
                      {state.stats.truncated && (
                        <div className="text-amber-400">Results truncated (too many nodes)</div>
                      )}
                    </div>
                  )}
