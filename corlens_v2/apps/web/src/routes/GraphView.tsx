...
import type { EdgeKind, GraphEdge, GraphNode, NodeKind } from "../lib/core-types.js";
...
function loadPersistedFilters(): PersistedFilters {
  ...
  try {
    ...
    return {
      disabledNodeKinds: Array.isArray(parsed.disabledNodeKinds) ? parsed.disabledNodeKinds : [],
      disabledEdgeKinds: Array.isArray(parsed.disabledEdgeKinds) ? parsed.disabledEdgeKinds : [],
...
function computeLayout(nodes: GraphNode[]): Node[] {
  ...
  issuerNodes.forEach((node, i) => {
    const offset = issuerNodes.length > 1 ? (i - (issuerNodes.length - 1) / 2) * 80 : 0;
    result.push({
      id: node.id,
      type: node.kind,
...
export default function GraphView() {
  ...
  useEffect(() => {
    ...
    window.localStorage.setItem(LEGEND_FILTER_STORAGE_KEY, JSON.stringify(payload));
  ...
  const filteredGraph = useMemo(() => {
    ...
    const visibleNodes = graphData.nodes.filter(
      (n) => !disabledNodeKinds.has(n.kind) && (showMinor || n.importance !== "secondary"),
    );
    const visibleIds = new Set(visibleNodes.map((n) => n.id));
    const visibleEdges = graphData.edges.filter(
      (e) => !disabledEdgeKinds.has(e.kind) && visibleIds.has(e.source) && visibleIds.has(e.target),
    );
  ...
  const seedLabel =
    statusData?.seedLabel ?? shortenAddress(statusData?.seedAddress ?? analysisId ?? "");

  return (
    <div className="flex h-[calc(100vh-var(--token-layout-navbarHeight))] flex-col">
      <GraphHeader seedLabel={seedLabel} stats={stats}>
        {minorCount > 0 && (
          <Button
            variant="secondary"
            size="sm"
      ...
      <div className="relative flex-1">
        ...
        >
          ...
          <MiniMap
            ...
            nodeColor={(n) => {
              ...
              return NODE_COLORS[gn.kind] ?? "var(--token-colors-border-default)";
            }}
            style={{
              background: "var(--token-colors-bg-secondary)",
              border: "1px solid var(--token-colors-border-subtle)",
        ...
        {!chatOpen && <FloatingGraphChatButton onOpen={() => setChatOpen(true)} />}
        ...
        {chatOpen && analysisId && (
          <GraphChatDrawer analysisId={analysisId} onClose={() => setChatOpen(false)} />
        )}
      </div>
    </div>
  );
