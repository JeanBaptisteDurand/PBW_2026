...
describe("chat.service", () => {
  it("embeds the user query, retrieves top-k context, and generates an answer", async () => {
    const ai = {
      ...
      complete: vi.fn().mockResolvedValue({ content: "The seed appears to be a healthy issuer.", tokensIn: 100, tokensOut: 30 }),
    };
    const repo = {
      searchByEmbedding: vi.fn().mockResolvedValue([
        { id: "doc-1", analysisId: "a-1", content: "issuer with high trust lines", metadata: {}, distance: 0.1 },
      ]),
