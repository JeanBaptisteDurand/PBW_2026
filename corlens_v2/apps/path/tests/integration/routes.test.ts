...
describe("path routes", () => {
  ...
  it("POST /api/analyze with bad address returns 400", async () => {
    const res = await app.inject({
      method: "POST",
      url: "/api/analyze",
      payload: { seedAddress: "not-an-address", depth: 1 },
    });
  ...
  it("POST /api/analyze creates a queued analysis for a valid address", async () => {
    const res = await app.inject({
      method: "POST",
      url: "/api/analyze",
      payload: { seedAddress: "rMxCKbEDwqr76QuheSUMdEGf4B9xJ8m5De", depth: 1 },
    });
    expect(res.statusCode).toBe(200);
    const body = res.json();
  ...
  it("GET /api/analysis/<unknown> returns 404", async () => {
    const res = await app.inject({
      method: "GET",
      url: "/api/analysis/00000000-0000-0000-0000-000000000000",
    });
  ...
  it("GET /api/history/<address> returns the history shape", async () => {
    const res = await app.inject({
      method: "GET",
      url: "/api/history/rMxCKbEDwqr76QuheSUMdEGf4B9xJ8m5De",
    });
    expect(res.statusCode).toBe(200);
    const body = res.json();
