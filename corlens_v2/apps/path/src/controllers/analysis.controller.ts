...
export async function registerAnalysisRoutes(
  ...
  typed.get(
    "/api/analysis/:id",
    {
      schema: {
        ...
        response: { 200: pp.AnalysisSummary, 404: ErrorResp },
        tags: ["analysis"],
      },
    },
    async (req, reply) => {
  ...
  typed.get(
    "/api/analysis/:id/graph",
    {
      schema: {
        params: z.object({ id: z.string().uuid() }),
        response: { 200: pp.GraphResponse, 404: ErrorResp },
        tags: ["analysis"],
      },
    },
    async (req, reply) => {
      ...
      const flagsByNode = new Map<
        string,
        Array<{ flag: string; severity: "HIGH" | "MED" | "LOW"; detail: string; data: unknown }>
      >();
      for (const f of flags) {
        const list = flagsByNode.get(f.nodeId) ?? [];
        list.push({
          flag: f.flag,
          severity: f.severity as "HIGH" | "MED" | "LOW",
          detail: f.detail,
          data: f.data,
        });
        flagsByNode.set(f.nodeId, list);
      }
      const stats = statsFromSummary(a.summaryJson) ?? {
  ...
  typed.get(
    "/api/analysis/:id/explanations",
    {
      schema: {
        params: z.object({ id: z.string().uuid() }),
        response: { 200: pp.ExplanationsResponse, 404: ErrorResp },
        tags: ["analysis"],
      },
    },
    async (req, reply) => {
