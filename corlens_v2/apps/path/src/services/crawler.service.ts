...
export function createCrawlerService(opts: CrawlerServiceOptions) {
  return {
    async crawl(seedAddress: string, seedLabel: string | null): Promise<CrawlResult> {
      ...
      const accountInfo = await safe(
        () => md.accountInfo(seedAddress) as Promise<Record<string, unknown>>,
        {} as Record<string, unknown>,
      );
      const trustLinesRaw = await safe(
        () => md.trustLines(seedAddress, { limit: 2000 }) as Promise<{ lines?: unknown[] }>,
        { lines: [] },
      );
      const accountObjects = await safe(
        () =>
          md.accountObjects(seedAddress) as Promise<{ result?: { account_objects?: unknown[] } }>,
        { result: { account_objects: [] } },
      );
      const accountTxs = await safe(
        () =>
      ...
      const gateway = await safe(
        () =>
          md.gatewayBalances(seedAddress) as Promise<{
            result?: { obligations?: Record<string, unknown> };
          }>,
        { result: { obligations: {} } },
      );
      const noripple = await safe(
        () => md.noripple(seedAddress) as Promise<{ result?: { problems?: unknown[] } }>,
        { result: { problems: [] } },
      );
      ...
      const txs = (accountTxs.result?.transactions ?? []) as Array<{
      ...
      const ammPool = await safe(
        () => md.ammByAccount(seedAddress) as Promise<{ result?: unknown }>,
        { result: null },
      );
