...
6. **corridor** — port catalog (move to JSON seed), scanner, RAG. All XRPL calls now go through market-data; all LLM calls go through ai-service. ✓ Implemented per [`docs/superpowers/plans/2026-05-09-corridor-service.md`](../plans/2026-05-09-corridor-service.md). Catalog seeded from JSON; scanner calls market-data; RAG calls ai-service. Currency-meta endpoint (`GET /api/corridors/currency-meta[/:code]`, 66 currencies seeded from v1 catalog) + chat retrieval (`GET /api/corridors/chat/:chatId`) added per [`docs/superpowers/plans/2026-05-11-backend-gap-fill.md`](../plans/2026-05-11-backend-gap-fill.md) (Phases 1–2, 8).
7. **path** — port BFS / graph / risk engine. Split `graphBuilder.ts` per node kind. Same external rerouting. ✓ Implemented per [`docs/superpowers/plans/2026-05-09-path-service.md`](../plans/2026-05-09-path-service.md) and Phase C of [`docs/superpowers/plans/2026-05-09-completion-roadmap.md`](../plans/2026-05-09-completion-roadmap.md). 19 risk flags + 21-node graph builder ported from v1; full 17-RPC crawler parity (Phase C.1); BFS depth 2/3 with hub picker + concurrency pool + maxNodes cap (Phase C.2); /api/history/stream SSE port of v1 historyOrchestrator + historyCrawler (Phase C.3). Risk-engine reachable from the agent via `POST /api/risk-engine/quick-eval` (HMAC-protected, 30s LRU/FIFO cache) + chat retrieval (`GET /api/analysis/:id/chat`) added per [`docs/superpowers/plans/2026-05-11-backend-gap-fill.md`](../plans/2026-05-11-backend-gap-fill.md) (Phases 4, 7).
8. **agent** — port the 9 phases as separate files. The orchestrator becomes a thin loop. ✓ Implemented per [`docs/superpowers/plans/2026-05-09-agent-service.md`](../plans/2026-05-09-agent-service.md) and Phase D of [`docs/superpowers/plans/2026-05-09-completion-roadmap.md`](../plans/2026-05-09-completion-roadmap.md). 9 Phase strategies (one file per phase), thin orchestrator iterating phases as live async generators (Phase D.1+D.2); SafePathRun.riskScore + auditHash columns added (Phase D.5); PDF compliance rendering via pdfkit + audit-hash footer (Phase D.3); GET /api/compliance/:id/pdf premium-gated + GET /api/compliance/verify public (Phase D.4); inter-service security in place — every connector signs via @corlens/clients hmacSigner, identity exposes /internal/premium-status HMAC-protected, identity publishes payment.confirmed via CompositeEventBus to ai-service + agent /events/:name receivers (Phase E). Backend gap-fill follow-ups closed per [`docs/superpowers/plans/2026-05-11-backend-gap-fill.md`](../plans/2026-05-11-backend-gap-fill.md): per-hop risk classification + `account-crawled` producer wired via the new `path:/api/risk-engine/quick-eval` endpoint (Phases 4–5); local 10-currency embed replaced with `corridor.getCurrencyMeta()` HTTP calls in agent Phase 01 (Phase 3); entity-compliance routes shipped at `/api/compliance/analysis/:id[ /pdf ]` reusing the `requirePremium` preHandler (Phase 6). Still open: HMAC verification on gateway-public routes deferred to Phase F when service-internal traffic moves to `/internal/*` path prefixes.
9. **web** — port the SPA, regenerate API client from `@corlens/contracts`. Keep Crossmark. ✓ Implemented per [`docs/superpowers/specs/2026-05-15-web-spa-design.md`](2026-05-15-web-spa-design.md) + [`docs/superpowers/plans/2026-05-15-web-spa.md`](../plans/2026-05-15-web-spa.md). Sub-spec #2 shipped across 16 commits on `refacto`: backend `GET /api/analyses` precondition + a contract fix for `corridor.ActorEntry.direction` + a corridor controller-registration-order fix; full `apps/web/` scaffold (Vite 5 + React 18 + RR v7 + Tailwind 3 + TanStack Query 5) wired into docker-compose + Caddy SPA fallback; typed per-service API clients (identity/corridor/path/agent) consuming `@corlens/contracts`; Crossmark SIWE two-step (challenge → verify) replacing v1's unauthenticated `POST /api/auth/connect`; design tokens + UI primitives + Layout/Navbar/GlobalStarfield ported verbatim; Landing Three.js scene (~1.8 kLOC + 26 MB GLB/MOV assets); all 14 v1 routes ported (Landing, Home, Analyze, GraphView, History, CorridorHealth, CorridorDetail, CorridorRoute, SafePath, Premium, Account, ApiDocs, Chat, ComplianceView) plus a `@corlens/core`-compat layer in `apps/web/src/lib/core-types.ts` + `apps/web/src/api/index.ts` that adapts v1-shape calls onto the v2 typed contracts. 10/10 routes return 200 at `localhost:8080`; 12/12 Playwright smoke tests pass (10 route mounts + 2 backend hits via Caddy).
10. **mcp-server** — regenerate tool defs from `@corlens/contracts`. ✓ Implemented per [`docs/superpowers/plans/2026-05-09-steps-9-12.md`](../plans/2026-05-09-steps-9-12.md). 7 MCP tools wired to v2 gateway (list_corridors, get_corridor, ask_corridor, analyze_address, ask_analysis, run_safe_path, get_partner_depth — the 7th added per [`docs/superpowers/plans/2026-05-11-backend-gap-fill.md`](../plans/2026-05-11-backend-gap-fill.md) Phase 9). Parity with v1 reached.
