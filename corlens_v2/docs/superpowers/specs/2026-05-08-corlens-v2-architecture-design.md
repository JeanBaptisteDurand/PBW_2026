...
7. **path** — port BFS / graph / risk engine. Split `graphBuilder.ts` per node kind. Same external rerouting.
8. **agent** — port the 9 phases as separate files. The orchestrator becomes a thin loop.
